...
- See `.env.example` for full list. Local config goes in `.env.local`.
...
5. **AI Analysis** — Gemini call if signals warrant it (`lib/ai/gemini.ts`), with template-based fallback (`lib/ai/fallback.ts`)
...
- `fallback.ts` — Template-based fallback when AI fails or is in mock mode
